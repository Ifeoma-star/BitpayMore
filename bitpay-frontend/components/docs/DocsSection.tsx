    ...
    <div className="space-y-6">
      <div className="border-b pb-4">
        <div className="flex items-center gap-3 mb-2">
          <h1 className="text-3xl font-bold bg-gradient-to-r from-brand-pink to-brand-teal bg-clip-text text-transparent">
            {title}
          </h1>
          {badge && (
...
interface InfoCardProps {
  title: string;
  children: ReactNode;
  variant?: 'default' | 'warning' | 'success' | 'info' | 'pink' | 'teal';
}
...
export function InfoCard({ title, children, variant = 'default' }: InfoCardProps) {
  const variants = {
    ...
    teal: 'border-brand-teal/20 bg-brand-teal/5',
  };
